...
@app.before_request
def to_allowed_host():
    # Allow requests from localhost (direct browser access)
    if request.remote_addr == '127.0.0.1':
        return
    
    # Allow requests from Docker containers using host.docker.internal
    if 'host.docker.internal' in request.headers.get('Host', ''):
        return
    
    # Allow requests with host.docker.internal in the URL
    if 'host.docker.internal' in request.url_root:
        return
